...
export const api = {
  get: async <T = any>(path: string, opts?: ReqOpts) =>
    request<T>(path, { method: 'GET', headers: opts?.headers, signal: opts?.signal }),
  ...
  delete: async <T = any>(path: string, opts?: ReqOpts) =>
    request<T>(path, { method: 'DELETE', headers: opts?.headers, signal: opts?.signal }),

  upload: async <T = any>(path: string, formData: FormData, opts?: { signal?: AbortSignal }) => {
    const url = `${API_BASE}${path}`;
    const res = await fetch(url, { method: 'POST', body: formData, signal: opts?.signal });
    if (!res.ok) {
      const text = await res.text();
      let body: unknown = text;
