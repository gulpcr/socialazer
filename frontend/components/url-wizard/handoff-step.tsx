...
import { ArrowLeft, Film, Clock, ImageIcon, Mic, CheckCircle2 } from "lucide-react"
...
export function HandoffStep({ script, voice, audioUrl, onBack }: HandoffStepProps) {
  const totalAssets = script.scenes.length
  const assetsReady = script.scenes.filter((s) => s.imageUrl).length

  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-semibold tracking-tight">Ready to Edit</h2>
        <p className="mt-1 text-muted-foreground">Your video is prepared and ready to open in the visual editor</p>
      </div>
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex items-center gap-3 rounded-lg border p-4">
              ...
              <div>
                <p className="text-2xl font-semibold">{script.totalDuration}s</p>
        ...
        <CardContent>
          <div className="space-y-3">
            {[
              ...
              { label: "Background images/visuals", ready: assetsReady === totalAssets },
              { label: "Voice-over audio", ready: !!audioUrl },
              { label: "Animation presets applied", ready: true },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                <CheckCircle2 className={`h-5 w-5 ${item.ready ? "text-green-500" : "text-muted-foreground"}`} />
